...
def createStudent(student_id, name):
    ...
    csv_reader = []
    with open("./db/student.csv", "r", newline = "\n") as f:
        csv_reader = list(csv.reader(f, delimiter=","))
    with open("./db/student.csv", "a", newline = "\n") as f:
    ...
    with open("./db/batch.csv", "r", newline = "\n") as f:
        csv_reader = list(csv.reader(f, delimiter=","))
    check = 0
    for i in range(0, len(csv_reader)):
        if(csv_reader[i][0] == batch_id):
            ...
            if(csv_reader[i][4] == ""):
                ...
                csv_reader[i][4] = csv_reader[i][4] + ":" + student_id
            df = pandas.read_csv("./db/batch.csv")
            df.loc[i-1, "list_of_students"] = csv_reader[i][4]
            df.to_csv("./db/batch.csv", index = False)
    if(check == 0):
        ...
        for j in range(0, len(courses)):
            if(csv_reader[i][0] == courses[j]):
                if(csv_reader[i][2] == ""):
                    ...
                    temp[student_id] = 0
                    csv_reader[i][2] = json.dumps(temp)
                df = pandas.read_csv("./db/course.csv")
                df.loc[i-1, "marks_obtained"] = csv_reader[i][2]
                df.to_csv("./db/course.csv", index = False)
...
def updateStudent(ostudent_id):
    ...
    while(True):
        ...
        if(x == 0):
            ...
            df = pandas.read_csv("./db/student.csv")
            df.loc[i-1, "Name"] = name
            df.to_csv("./db/student.csv", index = False)
        elif(x == 2):
